...
class AssetFrontEnd
{
	...
	public function new()
	{
		...
		if (sys.FileSystem.exists(directory) == false) throw 'Error finding custom asset directory:"$directory" from given path: $rawPath';
	...
	public dynamic function getAssetUnsafe(id:String, type:FlxAssetType, useCache = true):Null<Any>
	{
		...
		#else
		if (useOpenflAssets(id)) return getOpenflAssetUnsafe(id, type, useCache);
		...
		final asset:Any = switch type
		{
			...
			case IMAGE:
				...
				if (canUseCache) Assets.cache.setBitmapData(id, bitmap);
				bitmap;
			case SOUND:
				final sound = Sound.fromFile(getPath(id));
				if (canUseCache) Assets.cache.setSound(id, sound);
				sound;
			case FONT:
				final font = Font.fromFile(getPath(id));
				if (canUseCache) Assets.cache.setFont(id, font);
				font;
		}
	...
	public function getAsset(id:String, type:FlxAssetType, useCache = true, ?logStyle:LogStyle):Null<Any>
	{
		...
		{
			if (logStyle == null) logStyle = LogStyle.ERROR;
		...
		if (exists(id, type))
		{
			if (isLocal(id, type)) return getAssetUnsafe(id, type, useCache);
	...
	public dynamic function loadAsset(id:String, type:FlxAssetType, useCache = true):Future<Any>
	{
		...
		#else
		if (useOpenflAssets(id)) return loadOpenflAsset(id, type, useCache);
	...
	public dynamic function exists(id:String, ?type:FlxAssetType)
	{
		...
		#else
		if (useOpenflAssets(id)) return Assets.exists(id, type.toOpenFlType());
	...
	public dynamic function isLocal(id:String, ?type:FlxAssetType, useCache = true)
	{
		...
		#else
		if (useOpenflAssets(id)) Assets.isLocal(id, type.toOpenFlType(), useCache);
	...
	inline function addSoundExt(id:String)
	{
		...
		if (needsExt) return id + defaultSoundExtension;
